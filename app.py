...
MARKET_INDEX_DESC = {
    "경기동향": (
        "**📈 사용 지표**\n"
        "- **USSLIND (미국 경기선행지수)** — Conference Board 발표. 향후 6~12개월 경기 방향을 예측하는 종합 선행지표. 상승하면 경기 회복·확장 신호.\n"
        "- **UNRATE (실업률)** — 미국 노동시장 건전성 지표. 하락하면 고용 개선(경기 호황), 상승하면 경기 둔화 신호."
    ),
    "통화_재정정책": (
        "**📈 사용 지표**\n"
        "- **FEDFUNDS (미국 기준금리)** — 연준(Fed)이 설정하는 단기 정책금리. 인상 시 긴축(대출·소비 억제), 인하 시 완화(유동성 공급).\n"
        "- **M2SL (M2 통화량)** — 현금·요구불예금·저축예금 등 포함한 광의 통화량. 증가율이 높으면 유동성 확대, 감소하면 긴축 기조."
    ),
    "지정학_불확실성": (
        "**📈 사용 지표**\n"
        "- **USEPUINDXD (경제정책 불확실성 지수, EPU)** — 뉴스 기사 빈도 기반으로 산출. 수치가 높을수록 정치·정책 관련 불확실성 확대. "
        "실제 지정학 리스크(전쟁·분쟁 등)의 직접 지표는 아니며, 정책 불확실성의 대리 지표(proxy)로 사용."
    ),
    "자본시장_정책": (
        "**📈 사용 지표**\n"
        "- **VIX (CBOE 변동성 지수)** — S&P 500 옵션 가격에서 도출한 시장 기대 변동성. '공포 지수'라고도 불림. "
        "20 이하면 시장 안정, 30 이상이면 높은 불안감을 의미. 공매도 규제·세제 등 실제 자본시장 정책의 직접 지표는 아닌 간접 프록시."
    ),
}
        ...
        try:
            ...
            for key, label in MARKET_LABELS.items():
                with st.expander(f"🔍 {label}"):
                    st.markdown(MARKET_INDEX_DESC.get(key, ""), unsafe_allow_html=False)
                    st.divider()
